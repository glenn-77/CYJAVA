...
public class AffichageArbre {
    ...
    private void construireNiveaux(Personne personne, Map<Integer, List<Personne>> niveaux, int niveau) {
        ...
        if (personne.getPere() != null) construireNiveaux(personne.getPere(), niveaux, niveau - 1);
        if (personne.getMere() != null) construireNiveaux(personne.getMere(), niveaux, niveau - 1);
        for (Personne enfant : personne.getEnfants()) construireNiveaux(enfant, niveaux, niveau + 1);
    }
    ...
    private void dessinerArbreEtLiens(Group group, Personne personne, int profondeur) {
        if (personne == null || !positions.containsKey(personne) || profondeur > MAX_PROFONDEUR) return;
    ...
    private void dessinerLien(Group group, Personne enfant, Personne parent) {
        if (!positions.containsKey(enfant) || !positions.containsKey(parent)) return;

        double[] posEnfant = positions.get(enfant);
        double[] posParent = positions.get(parent);

        Line ligne = new Line(posParent[0], posParent[1] + 25, posEnfant[0], posEnfant[1] - 25);
